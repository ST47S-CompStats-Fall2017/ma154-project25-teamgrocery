...
set.seed(888)
p <- 15000/(125497040 - 123926072) # Proportion we want to sample (total, num. before Aug. 1 2016)
setkey(train, date)
...
#test_const = 100
#item_ids = unique(last_two_weeks$id)[1:test_const]
#store_ids = 1:54
...
item_store_sales = data.table(matrix(0, nrow=length(store_ids), ncol=length(item_ids)))
colnames(item_store_sales) = paste(item_ids)
for (i in 1:nrow(last_two_weeks[1:test_const])) {
  obs = last_two_weeks[i,]
  item_id = obs$id
  store_id = obs$store_nbr
  item_store_sales[store_id, item_id := item_store_sales[store_id, eval(item_id)] + 1]
}

# Actual matrix of days open for every store (unnecessary now)
...
# Instead:
# assumes all stores are open for all days
...
colnames(id_frequency) = c("id", "store_days_sold", "store_days")
for (i in 1:length(item_ids)) {
  total_transaction_days = sum(item_store_sales[i]) # for some reason this somes the column
  days_open_of_stores_w_item = 0
  for (j in 1:length(store_ids)) {
    if (item_store_sales[j, eval(i)] > 0) {
      days_open_of_stores_w_item = days_open_of_stores_w_item + days_open[1,store_ids[j]]
    }
  }
  print(total_transaction_days)
  print(days_open_of_stores_w_item)
  id_frequency[i, 2] = total_transaction_days
  id_frequency[i, 3] = days_open_of_stores_w_item
}

fwrite(id_frequency, "id_frequency.csv")

popular_items = filter(id_frequency, store_days_sold/store_days >= 0.95)
popular_items_ids = popular_items$id

popular_train = subset(train, id %in% popular_items_ids)
unpopular_train = subset(train, !(id %in% popular_items_ids))

popular_test = subset(test, id %in% popular_items_ids)
unpopular_test = subset(test, !(id %in% popular_items_ids))
```
...
s_p_indices <- sample(1:nrow(s_popular), 0.7*nrow(s_popular))
s_p_train <- s_popular[s_p_indices]
s_p_test <- s_popular[-s_p_indices]

# Train/test data for unpopular items
s_np_indices <- sample(1:nrow(s_not_popular), 0.7*nrow(s_not_popular))
s_np_train <- s_not_popular[s_np_indices]
s_np_test <- s_not_popular[-s_np_indices]

# Variables to use for the random forest
s_p_cols <- s_np_cols <- c("unit_sales", item_families, paste("Cluster",store_clusters), store_cities, paste("Store Type", store_types), weekday_names)
#s_p_cols <- c(s_p_cols, "popularity") TODO once Justin adds ARIMA-generated column
...
# C. Victor - code for splitting sandbox based off popularity
```{r}
...
s_popular <- s_all[popular == TRUE]
s_not_popular <- s_all[popular == FALSE]
