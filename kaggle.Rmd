...
for (i in 1:(ncol(s_train)-1)){
eval(parse(text = paste(paste(names(s_train)[i],"_sales_plot = ",sep = ""),"s_train %>% group_by(",names(s_train)[i],") %>% summarize(sales = sum(unit_sales)) %>% ggplot() + geom_col(aes(x =",names(s_train)[i],", y = sales))")))
}
store_cluster_sales_plot
