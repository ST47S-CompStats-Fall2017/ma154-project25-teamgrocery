...
```{r}
train_ids = unique(train_last2weeks$id)
transaction_recorded = rep(0,length(train_ids))
id_frequency = data.table(train_ids, transaction_recorded)
colnames(id_frequency) = c("id", "nbr_transactions")

total_store_train = 0
for (cur_store_nbr in stores$store_nbr) {
  total_store_train = total_store_train + 1
  store_train = filter(train_last2weeks, train_last2weeks$store_nbr == cur_store_nbr)
  for (cur_date in unique(store_train$date)) {
    total_store_train = total_store_train + 1
    store_day_train = filter(store_train, store_train$date == cur_date)
    for (cur_id in unique(store_day_train$id)) {
      id_frequency[id==cur_id]$nbr_transactions = id_frequency[id==cur_id]$nbr_transactions + 1
    }
  }
}
id_frequency[, prop_store_train := nbr_transactions/total_store_train]
View(id_frequency)
```
...
THIS CHUNK: Check if there is a daily trend
```{r}

avg_sales_data <- sandbox[,j = .(total_sales = sum(unit_sales), n = .N, avg_sales = sum(unit_sales)/.N), by = date]
avg_sales_data$ID <- seq.int(nrow(avg_sales_data))
ggplot(data = avg_sales_data) + geom_line(aes(x = ID, y = avg_sales))

# IMPORTANT - shows that avg. sales go up a bit as total sales go up
plot(avg_sales ~ total_sales, data = avg_sales_data) 
sales_lm <- lm(avg_sales ~ total_sales, data = avg_sales_data)
abline(sales_lm)
sales_lm


week_sales_data <- avg_sales_data[,j = .(total_sales, n, week = 1 + as.integer(ID/7))]
week_sales_data <- week_sales_data[,.(total_sales, n, avg_sales = sum(total_sales) / sum(n)),by = week]

wsd_sum <- week_sales_data[,j=.(total=sum(total_sales),N = sum(n)), by=.(week %% 7)]

ggplot(data = week_sales_data) + geom_line(aes(x = week, y = avg_sales))

adf.test(avg_sales_data$avg_sales, k = 20)
adf.test(week_sales_data$avg_sales, k = 20)
